...
export class McpHub {
		...
		try {
			...
			try {
				...
				HostProvider.window.showMessage({
					...
					message: "MCP 设置格式无效。请确保设置符合正确的 JSON 格式。",
				})
				return undefined
			}
	...
	async restartConnection(serverName: string): Promise<void> {
		...
		if (config) {
			...
			try {
				...
				HostProvider.window.showMessage({
					type: ShowMessageType.INFORMATION,
					message: `${serverName} MCP 服务器已连接`,
				})
			} catch (error) {
				Logger.error(`Failed to restart connection for ${serverName}:`, error)
				HostProvider.window.showMessage({
					type: ShowMessageType.ERROR,
					message: `连接 ${serverName} MCP 服务器失败`,
				})
			}
		}
	...
	public async toggleServerDisabledRPC(serverName: string, disabled: boolean): Promise<McpServer[]> {
		try {
			...
			HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: `更新服务器状态失败：${error instanceof Error ? error.message : String(error)}`,
			})
			throw error
		}
	}
	...
	public async updateServerTimeoutRPC(serverName: string, timeout: number): Promise<McpServer[]> {
		...
		try {
			...
			HostProvider.window.showMessage({
				type: ShowMessageType.ERROR,
				message: `更新服务器超时时间失败：${error instanceof Error ? error.message : String(error)}`,
			})
			throw error
		} finally {
			// Clear flag after a delay to ensure file watcher event has been processed
