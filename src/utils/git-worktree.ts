...
export async function createWorktree(
	...
	try {
		...
		let message = `工作树已创建在 ${worktreePath}`
		if (copiedCount > 0) {
			message += `（已从 .worktreeinclude 复制 ${copiedCount} 个文件）`
		}
		if (copyErrors.length > 0) {
			message += `。部分文件复制失败：${copyErrors.slice(0, 3).join(", ")}`
			if (copyErrors.length > 3) {
				message += `，以及另外 ${copyErrors.length - 3} 个`
			}
		}
...
export async function deleteWorktree(cwd: string, path: string, force: boolean = false): Promise<WorktreeResult> {
	...
	try {
		...
		return {
			success: true,
			message: `位于 ${path} 的工作树已移除`,
		}
	} catch (error) {
		return {
			success: false,
			message: `移除工作树失败：${error instanceof Error ? error.message : String(error)}`,
		}
	}
}
