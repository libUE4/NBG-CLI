...
export async function reconstructTaskHistory(showNotifications = true): Promise<TaskReconstructionResult | null> {
	try {
		...
		const proceed = await HostProvider.window.showMessage({
			...
			message:
				"这将从现有任务数据重建任务历史。此操作会备份当前任务历史，并尝试从任务文件夹恢复。是否继续？",
			options: {
				items: ["是，重建", "取消"],
			},
		})

		if (proceed?.selectedOption !== "是，重建") {
			return null
		}
		...
		if (showNotifications) {
			if (result.errors.length > 0) {
				const errorMessage = `重建已完成，但存在警告：\n- 已重建：${result.reconstructedTasks} 个任务\n- 已跳过：${result.skippedTasks} 个任务\n- 错误数：${result.errors.length}\n\n前几条错误：\n${result.errors.slice(0, 3).join("\n")}`
				...
				HostProvider.window.showMessage({
					type: ShowMessageType.INFORMATION,
					message: `任务历史重建成功！已找到并恢复 ${result.reconstructedTasks} 个任务。`,
				})
			}
		}
