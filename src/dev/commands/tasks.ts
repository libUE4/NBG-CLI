	...
	return [
		vscode.commands.registerCommand("cline.dev.expireMcpOAuthTokens", async () => {
			try {
				...
				if (!secretsJson) {
					vscode.window.showInformationMessage("未找到 MCP OAuth 密钥，没有服务器已认证")
					return
				}
				...
				const action = await vscode.window.showInformationMessage(
					`已使 ${expiredCount} 个 MCP OAuth token 过期。重新加载窗口以测试 token 刷新流程。`,
					"重新加载窗口",
					"取消",
				)
				...
				vscode.window.showErrorMessage(`使 token 过期失败：${error}`)
