...
function toGatewayProviderMetadata(
	metadata: Record<string, unknown> | undefined,
): GatewayProviderMetadata | undefined {
	return metadata as GatewayProviderMetadata | undefined;
}
...
function shouldUseBuiltinDirectly(input: {
	builtin: GatewayProviderRegistration | undefined;
	providerId: string;
	routedProviderId: string;
	collection: Awaited<ReturnType<typeof getProviderCollection>> | undefined;
	config: ProviderConfig;
}): boolean {
	return Boolean(
		input.builtin &&
			input.providerId === input.routedProviderId &&
			input.collection?.provider.source === "system" &&
			(!input.config.clientType ||
				input.config.clientType === input.collection.provider.client),
	);
}

function resolveCollectionFactory(input: {
	providerId: string;
	routedProviderId: string;
	collection: NonNullable<Awaited<ReturnType<typeof getProviderCollection>>>;
	config: ProviderConfig;
}): GatewayProviderFactory {
	const transport =
		input.config.clientType || input.providerId === input.routedProviderId
			? {
					client: input.config.clientType ?? input.collection.provider.client,
					protocol: input.collection.provider.protocol,
				}
			: { protocol: input.collection.provider.protocol };
	return resolveFactory(input.routedProviderId, transport);
}

function createCollectionRegistration(input: {
	providerId: string;
	routedProviderId: string;
	collection: NonNullable<Awaited<ReturnType<typeof getProviderCollection>>>;
	routedBuiltin: GatewayProviderRegistration | undefined;
	config: ProviderConfig;
}): GatewayProviderRegistration {
	return {
		manifest: {
			...
			defaultModelId: input.collection.provider.defaultModelId,
			models: Object.values(input.collection.models).map((model) =>
				toGatewayModelDefinition(input.providerId, model),
			),
			api: input.collection.provider.baseUrl,
			apiKeyEnv: input.collection.provider.env,
			metadata: toGatewayProviderMetadata(input.collection.provider.metadata),
		},
		defaults: {
			...(input.routedBuiltin?.defaults ?? {}),
			baseUrl:
				input.collection.provider.baseUrl ??
				input.routedBuiltin?.defaults?.baseUrl,
			apiKeyEnv:
				input.collection.provider.env ??
				input.routedBuiltin?.defaults?.apiKeyEnv,
		},
		createProvider: resolveCollectionFactory(input),
	};
}
...
async function resolveProviderRegistration(
	...
	const routedBuiltin = BUILTIN_PROVIDER_MAP.get(routedProviderId);
	return createCollectionRegistration({
		providerId,
		routedProviderId,
		collection,
		routedBuiltin,
