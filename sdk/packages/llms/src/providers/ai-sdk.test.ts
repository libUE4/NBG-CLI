...
import { extractReasoningMetadata, normalizeUsage } from "./ai-sdk";
...
describe("ai-sdk reasoning metadata", () => {
	it("extracts Anthropic thinking signatures from metadata-only deltas", () => {
		expect(
		...
		).toEqual({ signature: "sig_123" });
	});

	it("extracts redacted thinking data from reasoning starts", () => {
		expect(
			extractReasoningMetadata({
				...
				providerMetadata: {
					anthropic: {
						redactedData: "encrypted_payload",
					},
				},
			}),
		).toEqual({ redactedData: "encrypted_payload" });
	});

	it("extracts Bedrock reasoning signatures for replay", () => {
		expect(
			extractReasoningMetadata({
				type: "reasoning-delta",
				text: "",
				providerMetadata: {
					bedrock: {
						signature: "bedrock_sig",
					},
				},
			}),
		).toEqual({ signature: "bedrock_sig" });
	});

	it("keeps Gemini thought signatures on reasoning metadata", () => {
		expect(
			extractReasoningMetadata({
				type: "reasoning-delta",
				text: "thought",
		...
		).toEqual({ thoughtSignature: "gemini_sig" });
	});
});
