...
describe("composeAiSdkProviderOptions: family/provider thinking patches", () => {
	runCases([
		...
		{
			name: "openai-compatible relay Claude id without routing metadata stays OpenAI-compatible",
			request: {
			...
			expect: [
				{
					bucket: "openaiCompatible",
					has: { strictJsonSchema: false },
					lacks: ["thinking", "reasoning", "cache_control"],
				},
				{
					bucket: "anthropic",
					lacks: ["thinking", "reasoning", "cache_control"],
