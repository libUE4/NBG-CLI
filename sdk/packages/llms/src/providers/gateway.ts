...
class GatewayModelAdapter implements AgentModel {
	...
	stream(request: AgentModelRequest): Promise<AsyncIterable<AgentModelEvent>> {
		const requestedReasoning = normalizeGatewayReasoning(
		...
		const legacyEffort = normalizeGatewayReasoningEffort(reasoningEffort);
...
function normalizeGatewayReasoningEffort(
	value: unknown,
...
function normalizeGatewayReasoning(
	value: unknown,
): GatewayStreamRequest["reasoning"] | undefined {
	...
	const reasoning: NonNullable<GatewayStreamRequest["reasoning"]> = {};
	if (typeof input.enabled === "boolean") {
		reasoning.enabled = input.enabled;
	}
	const effort = normalizeGatewayReasoningEffort(input.effort);
	if (effort) {
		reasoning.effort = effort;
	}
	if (typeof input.budgetTokens === "number") {
		reasoning.budgetTokens = input.budgetTokens;
	}
