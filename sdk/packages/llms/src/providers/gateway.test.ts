...
describe("sdk-gateway", () => {
	...
	it("surfaces API detail fields from OpenAI-compatible error bodies", async () => {
		...
		expect(events.at(-1)).toEqual({
			type: "finish",
			reason: "error",
			error: "上游请求失败（HTTP 400）：缺少 instructions 字段",
		});
	});

	it("localizes common upstream quota and model errors", async () => {
		streamTextSpy
			.mockReturnValueOnce({
				fullStream: makeFailingStreamParts(
		...
		const quotaEvents = await collect(
			await gateway.stream({
				providerId: "openai-native",
				modelId: "gpt-5-mini",
				messages: baseMessages,
		...
		expect(quotaEvents.at(-1)).toEqual({
			type: "finish",
			reason: "error",
			error: "额度不足或触发限流",
		});
	...
	it("keeps AI SDK tool-error parts recoverable", async () => {
		...
		expect(toolCallEvents[1]).toMatchObject({
			...
			toolName: "run_commands",
			metadata: {
				inputParseError: expect.stringContaining("工具 run_commands 输入无效"),
				aiSdkToolError:
					"上游返回错误：工具 run_commands 输入无效：JSON 解析失败",
			},
		});
		expect(events.at(-1)).toEqual({
			type: "finish",
	...
	it("normalizes legacy xhigh thinking effort for OpenAI-compatible providers", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{ type: "finish", usage: { inputTokens: 1, outputTokens: 1 } },
			]),
	...
	it("streams AI SDK reasoning deltas that use the delta field", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{ type: "reasoning-delta", delta: "先思考。" },
		...
		expect(events).toContainEqual({ type: "text-delta", text: "你好。" });
	});

	it("keeps Anthropic reasoning signatures from metadata-only deltas", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{ type: "reasoning-delta", delta: "先思考。" },
				{
	...
	it("streams OpenAI-compatible reasoning text from nested delta fields", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{
					type: "text-delta",
					delta: { reasoning_content: "嵌套思考。" },
				},
				{ type: "text-delta", delta: { content: "回答。" } },
				{ type: "finish", usage: { inputTokens: 1, outputTokens: 1 } },
			]),
		});
		...
		expect(events).toContainEqual({ type: "text-delta", text: "回答。" });
	});
