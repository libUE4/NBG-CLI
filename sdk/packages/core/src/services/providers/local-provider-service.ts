...
export interface UpdateLocalProviderRequest {
	...
	modelCapabilities?: Record<string, ProviderCapability[] | undefined>;
}
...
function inferModelCapabilities(modelId: string): ProviderCapability[] {
	const normalized = modelId.toLowerCase();
	const capabilities = new Set<ProviderCapability>(["tools"]);
	if (
...
function buildProviderModels(
	...
	const usePerModelCapabilities = modelCapabilities !== undefined;
	return Object.fromEntries(
		modelIds.map((id) => {
			const perModelCapabilities = usePerModelCapabilities
				? (modelCapabilities[id] ?? inferModelCapabilities(id))
				: undefined;
			const modelSupportsVision =
				perModelCapabilities?.includes("vision") ??
				(!usePerModelCapabilities && supportsVision);
			const modelSupportsReasoning =
				perModelCapabilities?.includes("reasoning") ??
				(!usePerModelCapabilities && supportsReasoning);
			return [
				id,
				{
					id,
					...
					supportsAttachments: modelSupportsVision,
...
export async function updateLocalProvider(
	...
	const modelIds = await resolveModelIds({
		...
		apiKey: request.apiKey ?? existingSettings?.apiKey,
		headers:
