...
export interface ClineSystemPromptOptions
	...
	providerId?: string;
	/** Current runtime model ID, included so identity answers match routing */
	modelId?: string;
}
...
export function buildClineSystemPrompt(
	...
	return basePrompt
		...
		.replace("{{RUNTIME_PROVIDER_ID}}", providerId?.trim() || "unknown")
		.replace("{{RUNTIME_MODEL_ID}}", modelId?.trim() || "unknown")
		.replace(
			"{{CLINE_METADATA}}",
			isCline
