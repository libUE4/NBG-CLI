...
// Publishes nbg and all platform-specific binary packages to npm.
//
// Usage:
...
const wrapperPackageName = "nbg";

const expectedPlatformPackages = [
	"@nbg/cli-darwin-arm64",
	"@nbg/cli-darwin-x64",
	"@nbg/cli-linux-arm64",
	"@nbg/cli-linux-x64",
	"@nbg/cli-windows-arm64",
	"@nbg/cli-windows-x64",
] as const;
...
const platformTasks = Object.keys(binaries)
	.sort()
	.map(async (name) => {
		const dirName = name.replace("@nbg/", "");
...
const wrapperPackageJson = {
	...
	version,
	description: description || "NBG CLI",
