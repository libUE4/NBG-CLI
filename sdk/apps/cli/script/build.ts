...
console.log(`Building @nbg/cli v${version}`);
...
if (!buildOptions.skipSdkBuild) {
	...
	await $`bun -F @nbg/cli build`.cwd(rootDir);
}
...
async function buildCompiledBinary(input: {
	...
	const tmpDir = join("/tmp", `nbg-build-${input.dirName}`);
	const tmpOutfile = join(
		tmpDir,
		input.outfile.endsWith(".exe") ? "nbg.exe" : "nbg",
	);
...
for (const item of targets) {
	...
	const name = `@nbg/cli-${displayOs}-${item.arch}`;
	const dirName = `cli-${displayOs}-${item.arch}`;
	const binaryName = item.os === "win32" ? "nbg.exe" : "nbg";
		...
		`${JSON.stringify(
			{
				name,
				version,
				description: `NBG CLI binary for ${displayOs} ${item.arch}`,
				os: [item.os],
				cpu: [item.arch],
