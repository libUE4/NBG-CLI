...
// Post-install script for NBG CLI.
//
// Creates a hard link (or copy fallback) from the platform-specific binary
// to bin/.nbg for fast startup on subsequent runs.
//
...
function main() {
	...
	const arch = os.arch();
	const packageName = `@nbg/cli-${platform}-${arch}`;
	const binaryName = "nbg";
	...
	const binDir =
		path.basename(__dirname) === "script"
			...
			: path.join(__dirname, "bin");
	const target = path.join(binDir, ".nbg");
	...
	console.log(`Cached nbg binary at ${target}`);
}
