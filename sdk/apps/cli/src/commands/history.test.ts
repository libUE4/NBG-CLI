...
const mockedReadSessionMessagesArtifact = readSessionMessagesArtifact as ReturnType<
	typeof vi.fn
>;
const mockedListSessions = listSessions as ReturnType<typeof vi.fn>;
const mockedRenderHistoryStandalone = renderHistoryStandalone as ReturnType<
	typeof vi.fn
>;
...
describe("formatHistoryListLine", () => {
	it("includes checkpoint metadata when present", () => {
		...
		expect(line).toContain(
			"12/31/2025 16:00 mock-provider:mock-model [已完成] | $0.25 | hello world",
		);
	});
	...
	it("formats checkpoint detail text for the selected row footer", () => {
		...
		expect(detail).toContain("检查点");
		expect(detail).toContain("第 3 轮");
		expect(detail).toContain("共 3 个");
	});
	...
	it("omits cost for subscription-backed providers", () => {
		...
		expect(line).toContain("openai-codex:gpt-5.4 [已完成] | hello world");
...
describe("runHistoryList", () => {
	...
	it("defaults history listing to 50 rows", async () => {
		...
		expect(io.writeln).toHaveBeenCalledWith("未找到历史记录。");
	});
});
...
describe("runHistoryExport", () => {
	...
	it("fails when the session artifact is missing", async () => {
		...
		expect(io.writeErr).toHaveBeenCalledWith(
			"未找到会话 sess_missing，或该会话没有 messages.json",
		);
	});
});
