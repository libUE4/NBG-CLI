...
export async function checkForUpdates(
	...
	writeln(
		`${c.cyan}正在检查${includeKanban ? " NBG CLI 和 kanban 的" : ""}更新…${c.reset}`,
	);
	...
	try {
		...
		if (options.verbose) {
			...
			if (includeKanban) {
				...
				if (!kanbanInstallCommand) {
					writeln(
						`${c.dim}Kanban 安装器不可用（未找到 npm、pnpm 或 bun）${c.reset}`,
					);
				}
			}
		}
		...
		if (cliUpdateAvailable && latestVersion) {
			writeln(
				`${c.yellow}发现新版本：${c.reset} ${c.bold}${latestVersion}${c.reset}（当前：${currentVersion}）`,
			);
		}
		...
		if (cliUpdateAvailable && latestVersion) {
			if (!updateCommand) {
				writeln(
					`${c.dim}无法确定 NBG 更新命令。请使用你的包管理器手动更新。${c.reset}`,
				);
				hadFailure = true;
			} else {
				...
				try {
					...
					if (exitCode === 0) {
						...
						writeErr(
							`NBG 更新失败（退出码 ${exitCode}）。可尝试运行：${manualUpdateCommand.command}`,
						);
						hadFailure = true;
					}
				} catch (error) {
					...
					writeErr(
						`运行 NBG 更新命令失败 ${manualUpdateCommand.command}：${message}`,
					);
					hadFailure = true;
				}
			}
		...
		if (shouldUpdateKanban && kanbanInstallCommand && latestKanbanVersion) {
			...
			try {
				...
				if (exitCode === 0) {
					...
					writeErr(
						`Kanban 更新失败（退出码 ${exitCode}）。可尝试运行：${kanbanInstallCommand.displayCommand}`,
					);
					hadFailure = true;
				}
			} catch (error) {
				...
				writeErr(
					`运行 Kanban 更新命令失败 ${kanbanInstallCommand.displayCommand}：${message}`,
				);
				hadFailure = true;
			}
		}
