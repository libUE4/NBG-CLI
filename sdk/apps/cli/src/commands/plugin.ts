...
function normalizeRemotePluginFileUrl(
	...
	if (parsed.protocol !== "https:") {
		if (isGitHubFile || host === "raw.githubusercontent.com" || isPluginFile) {
			throw new Error(`远程插件文件 URL 必须使用 https：${source}`);
		}
		return null;
	}
	...
	if (host === "github.com" || host === "www.github.com") {
		...
		if (!isPluginFile) {
			throw new Error(`远程插件文件必须是 .js 或 .ts：${source}`);
		}
...
function toWrapperEntryPaths(
	...
	if (entries.length === 0) {
		throw new Error(`在 ${packageRoot} 中未找到插件入口文件`);
	}
	return entries.map(
...
function remotePluginSizeLimitError(url: string): Error {
	return new Error(
		`来自 ${url} 的远程插件文件超过 ${REMOTE_PLUGIN_MAX_BYTES} 字节限制`,
	);
}
...
async function installRemoteFile(
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				`从 ${parsed.url} 下载插件文件失败：${response.status}${suffix}`,
			);
		}
...
async function installLocalPackage(
	...
	if (!existsSync(absolutePath)) {
		throw new Error(`插件来源路径不存在：${absolutePath}`);
	}
...
function assertCanInstall(targetPath: string, force: boolean): void {
	if (existsSync(targetPath) && !force) {
		throw new Error(
			`插件已安装在 ${targetPath}。使用 --force 可替换它。`,
		);
	}
}
...
function replaceInstallPath(
	...
	if (!force) {
		throw new Error(
			`插件已安装在 ${installPath}。使用 --force 可替换它。`,
		);
	}
...
export async function installPlugin(
	...
	try {
		...
		if (entryPaths.length === 0) {
			throw new Error(`未找到 ${source} 的插件入口文件`);
		}
