...
function formatRecentSpawnedProcess(record: SpawnedProcessRecord): string {
	const pieces = [
		record.timestamp ?? "未知时间",
		record.component ?? "未知组件",
		record.pid ? `pid=${record.pid}` : undefined,
		record.detached === undefined
			? undefined
			: `已分离=${record.detached ? "是" : "否"}`,
		record.command,
	].filter(Boolean);
	return pieces.join(" | ");
}
...
function formatActiveConnector(record: ActiveConnectorRecord): string {
	const identity =
		record.type === "telegram"
			? `机器人=@${record.botUsername ?? "未知"}`
			: record.type === "discord"
				? `用户=${record.userName ?? "未知"} 应用=${record.applicationId ?? "未知"}`
				: `用户=${record.userName ?? "未知"}`;
	const pieces = [
		record.type,
		identity,
		`pid=${record.pid}`,
		`hub=${record.hubUrl}`,
		record.phoneNumberId ? `手机号=${record.phoneNumberId}` : undefined,
		record.port ? `端口=${record.port}` : undefined,
		record.baseUrl ? `URL=${record.baseUrl}` : undefined,
		record.startedAt ? `启动时间=${record.startedAt}` : undefined,
	].filter(Boolean);
	return pieces.join(" | ");
}
...
export async function runDoctorCommand(
	...
	if (!fix) {
		...
		writeln(`hub URL ${c.dim}${before.hubUrl ?? "无"}${c.reset}`);
		writeln(
			`hub 健康状态 ${c.dim}${before.hubHealthy ? "是" : "否"}${before.hubPid ? ` (pid=${before.hubPid})` : ""}${c.reset}`,
		);
		writeln(`hub 运行时长 ${c.dim}${before.hubUptime ?? "不可用"}${c.reset}`);
		...
		writeln(formatPidList("sidecar 进程", before.staleSidecarPids));
	...
	writeln(`已终止 sidecar 进程 ${c.dim}${killedSidecars}${c.reset}`);
	writeln(
	...
	writeln(
		`已清理 hub 启动锁 ${c.dim}${clearedArtifacts.startupLocks}${c.reset}`,
	);
	writeln(
		`已清理 hub 发现记录 ${c.dim}${clearedArtifacts.discovery}${c.reset}`,
	);
	writeln(`修复后 hub 健康状态：${after.hubHealthy ? "是" : "否"}`);
	writeln(formatPidList("剩余 hub 监听进程", after.listeningPids));
	writeln(
		formatPidList(
			"剩余 hub 启动锁",
	...
	writeln(formatPidList("剩余 CLI 进程", after.staleCliPids));
	writeln(formatPidList("剩余 sidecar 进程", after.staleSidecarPids));
	return 0;
}
...
export function createDoctorCommand(
	...
	const doctor = localizeCommander(new Command("doctor"))
		.description("诊断并修复本地进程问题")
		.exitOverride()
		.option("--cwd <path>", "工作区根路径", process.cwd())
		.option("--json", "以 JSON 输出")
		.option("-v, --verbose", "显示更多诊断详情")
