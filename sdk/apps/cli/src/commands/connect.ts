...
export async function runStopAllConnectors(io: ConnectIo): Promise<number> {
	...
	if (executed === 0) {
		io.writeln("[connect] 暂无适配器支持停止操作");
		return 0;
	}
	io.writeln(
		`[connect] 已停止进程=${stoppedProcesses} 会话=${stoppedSessions}`,
	);
	return 0;
}
...
export async function runStopConnector(
	...
	if (!connector) {
		io.writeErr(`未知连接适配器 "${adapterName}"`);
		return 1;
	}
	if (!connector.stopAll) {
		io.writeErr(`连接适配器 "${adapterName}" 不支持停止操作`);
		return 1;
	}
	const result: ConnectStopResult = await connector.stopAll(io);
	io.writeln(
		`[connect] ${connector.name} 已停止进程=${result.stoppedProcesses} 会话=${result.stoppedSessions}`,
	);
	return 0;
}
