...
import {
	formatThirdPartyAccountBalance,
...
export async function runInteractive(
	...
	const refreshInteractiveSessionPolicies = async (): Promise<void> => {
		...
		sessionPolicyRefresh = (async () => {
			...
			.catch((error) => {
				logCliError(config.logger, "交互策略刷新失败", {
					error,
				});
	...
	const loadDeferredInitialMessages = resumeSessionId?.trim()
		? async () => {
				try {
					...
					logCliError(config.logger, "交互启动失败", { error });
					throw error;
				}
			}
		: undefined;
	...
	tuiApp = await renderOpenTui({
		...
		loadAccountBalance: async () => {
			if (config.providerId === THIRD_PARTY_PROVIDER_ID) {
				const balance = await loadThirdPartyAccountBalance(
					providerSettingsManager,
				);
				return balance ? formatThirdPartyAccountBalance(balance) : undefined;
			}
			if (config.providerId === "cline") {
				const snapshot = await loadClineAccountSnapshot({
					config,
					clineApiBaseUrl: options?.clineApiBaseUrl,
				});
				return formatClineCredits(snapshot.displayedBalance);
			}
			return undefined;
		},
		...
		onSubmit: async (input, mode, delivery, attachments) => {
			try {
				...
				logCliError(config.logger, "交互回合失败", {
					error,
