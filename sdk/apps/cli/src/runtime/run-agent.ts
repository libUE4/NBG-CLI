...
function printModelProviderInfo(config: Config): void {
	const catalog = config.knownModels ? "实时" : "内置";
	const thinking = config.thinking ? "开启" : "关闭";
	...
	writeln(
		`${c.dim}[模型] 提供方=${providerId} 模型=${modelId} 目录=${catalog} 推理=${thinking} 模式=${mode}${c.reset}\n`,
	);
}
...
function emitAbortRequested(
	...
	if (config.outputMode === "json") {
		...
		writeln(`\n${c.dim}[中止] 已请求${c.reset}`);
	}
}

function emitTeamRestored(config: Config): void {
	const teamName = config.teamName ?? "（未知团队）";
	if (config.outputMode === "json") {
		emitJsonLine("stdout", { type: "team_restored", teamName });
		return;
	}
	writeln(
		`${c.dim}[团队] 已恢复 "${teamName}" 的持久化团队状态${c.reset}`,
	);
}
...
function printRunStats(
	...
	if (config.verbose) {
		...
		if (usage.cacheReadTokens) {
			tokenParts.push(`缓存读取 ${usage.cacheReadTokens}`);
		}
		if (usage.cacheWriteTokens) {
			tokenParts.push(`缓存写入 ${usage.cacheWriteTokens}`);
		}
		...
		if (
			...
			parts.push(`预估费用 ${formatUsd(usage.totalCost)}`);
		}
		...
		if (config.thinking) {
			writeln(
				`${c.dim}[推理] 分块=${reasoningChunkCount} 已隐藏=${redactedReasoningChunkCount}${c.reset}`,
			);
		}
	}
}
...
export async function runAgent(
	...
	void prewarmFileIndex(config.cwd).catch((error: unknown) => {
		logCliError(config.logger, "文件索引预热失败", { error });
	});
	...
	const abortAll = () => {
		...
		if (activeSessionId) {
			sessionManager
				.abort(activeSessionId, new Error("已请求中止命令行运行时"))
				.catch(() => {});
		}
		return true;
	};
	...
	try {
		...
		if (abortRequested || result.finishReason === "aborted") {
			if (timedOut) {
				writeErr(`运行在 ${config.timeoutSeconds}s 后超时`);
				process.exitCode = 1;
