...
> {
	constructor() {
		super("slack", "基于 RPC 运行时会话的 Slack webhook 桥接");
	}
	...
	protected override createCommand(): Command {
		return super
			...
			.option("--enable-tools", "为 Slack 会话启用工具")
			.option(
			...
			.option(
				"--base-url <url>",
				"Webhook 和 OAuth 回调用的公开 Base URL",
			)
			.addHelpText(
				"after",
				[
					"",
					"环境变量：",
					"  SLACK_BOT_TOKEN             单工作区机器人 token",
					"  SLACK_SIGNING_SECRET        Slack 签名密钥",
					"  SLACK_CLIENT_ID             OAuth client ID",
					"  SLACK_CLIENT_SECRET         OAuth client secret",
					"  SLACK_ENCRYPTION_KEY        可选安装信息加密密钥",
				].join("\n"),
			);
	}
	...
	protected override async runWithOptions(
		...
		if (
			await this.maybeRunInBackground({
				...
				formatBackgroundStartMessage: (pid) =>
					`[slack] 正在启动后台连接器 pid=${pid} user=${options.userName}`,
				foregroundHint:
					"[slack] 使用 `nbg connect slack -i ...` 可在前台运行",
				launchFailureMessage: "启动 Slack 后台连接器失败",
			})
		) {
			return 0;
		}
		...
		io.writeln(`[slack] 请配置 Slack webhook URL：${webhookUrl}`);
		io.writeln(
			`[slack] 请配置 Slack OAuth callback URL：${oauthCallbackUrl}`,
		);
