...
> {
	constructor() {
		super(
			"whatsapp",
			"基于 RPC 运行时会话的 WhatsApp Business webhook 桥接",
		);
	}
	...
	protected override createCommand(): Command {
		return super
			...
			.option("--access-token <token>", "Meta 访问 token")
			.option("--app-secret <secret>", "Meta 应用密钥")
			.option("--verify-token <token>", "Webhook 验证 token")
			...
			.option("--enable-tools", "为 WhatsApp 会话启用工具")
			.option(
	...
	protected override async runWithOptions(
		...
		if (
			await this.maybeRunInBackground({
				...
				formatBackgroundStartMessage: (pid) =>
					`[whatsapp] 正在启动后台连接器 pid=${pid} user=${options.userName}`,
				foregroundHint:
					"[whatsapp] 使用 `nbg connect whatsapp -i ...` 可在前台运行",
		...
		io.writeln(`[whatsapp] 请配置 WhatsApp webhook URL：${endpointUrl}`);
