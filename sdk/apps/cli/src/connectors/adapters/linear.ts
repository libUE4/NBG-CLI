...
const LINEAR_FIRST_CONTACT_MESSAGE = [
	"已连接。",
	"你的聊天历史会按此 Linear 身份单独隔离。",
	"如需投递线程详情，请发送 /whereami。",
].join("\n");
...
> {
	...
	protected override createCommand(): Command {
		return super
			...
			.option("--base-url <url>", "Webhook 配置使用的公开 Base URL")
			.addHelpText(
				"after",
				[
					"",
					"环境变量：",
					"  LINEAR_API_KEY             个人 API Key",
					"  LINEAR_CLIENT_ID           OAuth client ID",
					"  LINEAR_CLIENT_SECRET       OAuth client secret",
					"  LINEAR_ACCESS_TOKEN        预先获取的 access token",
	...
	protected override async runWithOptions(
		...
		if (
			await this.maybeRunInBackground({
				...
				formatBackgroundStartMessage: (pid) =>
					`[linear] 正在启动后台连接器 pid=${pid} user=${options.userName}`,
				foregroundHint:
					"[linear] 使用 `nbg connect linear -i ...` 可在前台运行",
				launchFailureMessage: "启动 Linear 后台连接器失败",
			})
		) {
			return 0;
		}
		...
		io.writeln(`[linear] 请配置 Linear webhook URL：${webhookUrl}`);
