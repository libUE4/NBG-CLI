...
> {
	constructor() {
		super("gchat", "基于 RPC 运行时会话的 Google Chat webhook 桥接");
	}
	...
	protected override createCommand(): Command {
		return super
			...
			.option("--enable-tools", "为 Google Chat 会话启用工具")
			.option(
			...
			.option("--use-adc", "使用 Google 应用默认凭据")
			...
			.addHelpText(
				...
				[
					...
					"  GOOGLE_CHAT_USE_ADC=true     使用应用默认凭据",
					"  GOOGLE_CHAT_PUBSUB_TOPIC     可选 Pub/Sub 主题",
					"  GOOGLE_CHAT_IMPERSONATE_USER 可选委派用户",
				].join("\n"),
			);
	}
	...
	protected override async runWithOptions(
		...
		if (
			await this.maybeRunInBackground({
				...
				formatBackgroundStartMessage: (pid) =>
					`[gchat] 正在启动后台连接器 pid=${pid} user=${options.userName}`,
				foregroundHint:
					"[gchat] 使用 `nbg connect gchat -i ...` 可在前台运行",
				launchFailureMessage:
					"启动 Google Chat 后台连接器失败",
			})
		) {
			return 0;
		}
		...
		io.writeln(`[gchat] 请配置 Google Chat App URL：${endpointUrl}`);
