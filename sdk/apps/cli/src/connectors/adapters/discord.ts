...
> {
	...
	protected override createCommand(): Command {
		return super
			...
			.option("--bot-token <token>", "Discord 机器人 token")
			...
			.option("--enable-tools", "为 Discord 会话启用工具")
			.option(
			...
			.option(
				"--base-url <url>",
				"Discord 交互 webhook 使用的公开 Base URL",
			)
			.addHelpText(
				"after",
				[
					...
					"  DISCORD_MENTION_ROLE_IDS    可选的逗号分隔角色 ID",
				].join("\n"),
			);
	}
	...
	protected override async runWithOptions(
		...
		if (
			await this.maybeRunInBackground({
				...
				foregroundHint:
					"[discord] 使用 `nbg connect discord -i ...` 可在前台运行",
		...
		if (!gatewayStartResponse.ok) {
			...
			io.writeErr(
				`启动 Discord gateway 监听失败：${await gatewayStartResponse.text()}`,
			);
			return 1;
		}
		...
		io.writeln(
			`[discord] 请配置 Discord interactions endpoint：${webhookUrl}`,
		);
		io.writeln(
			"[discord] gateway 监听已启动，可处理提及、回复、reaction 和私信",
		);
