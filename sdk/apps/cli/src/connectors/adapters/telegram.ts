...
async function fetchTelegramBotUsername(
	...
	if (!username) {
		throw new Error(
			"Telegram getMe 没有返回机器人用户名；请显式传入 --bot-username",
		);
	}
	return username;
}
...
async function deliverScheduledResult(input: {
	...
	if (input.status === "success" && input.sessionId) {
		...
		body = text?.trim()
			? text
			: `计划任务 "${schedule?.name ?? input.scheduleId}" 已完成，但没有找到助手回复文本。`;
	} else {
		body = `计划任务 "${schedule?.name ?? input.scheduleId}" 状态：${input.status}.${input.errorMessage ? `\n\n${input.errorMessage}` : ""}`;
	}
	try {
...
> {
	constructor() {
		super("telegram", "将 Telegram 机器人消息桥接到 RPC 聊天会话");
	}

	protected override createCommand(): Command {
		return super
			.createCommand()
			.usage("-k <TELEGRAM_BOT_TOKEN> [选项]")
			.option(
				"-m, --bot-username <name>",
				"Telegram 机器人用户名；省略时从 token 获取",
			)
			.option("-k, --bot-token <token>", "Telegram 机器人 token")
			.option("--provider <id>", "覆盖提供方")
			.option("--model <id>", "覆盖模型")
			.option("--api-key <key>", "覆盖提供方 API Key")
			.option("--system <prompt>", "覆盖系统提示词")
			.option("--cwd <path>", "运行时工作区 / cwd")
			.option("--mode <act|plan>", "智能体模式", "act")
			.option("-i, --interactive", "让连接器保持前台运行")
			...
			.addHelpText(
				...
				[
					...
					"  - 不传 -i 时，连接器会在后台启动。",
					"  - Telegram 会话默认启用工具。",
					"  - 省略机器人用户名时，会从 Telegram 机器人 token 自动发现。",
					"  - 提供方/模型默认使用 CLI 上次使用的提供方设置。",
				].join("\n"),
			);
	}
	...
	protected override async runWithOptions(
		...
		if (
			await this.maybeRunInBackground({
				...
				formatBackgroundStartMessage: (pid) =>
					`[telegram] 正在启动后台连接器 pid=${pid} bot=@${options.botUsername}`,
				foregroundHint:
					"[telegram] 使用 `nbg connect telegram -i ...` 可在前台运行",
		...
		io.writeln(
			`[telegram] 已连接为 @${options.botUsername} 模式=${telegram.runtimeMode} rpc=${rpcAddress} 提供方=${startRequest.provider} 模型=${startRequest.model} 工具=${startRequest.enableTools ? "开启" : "关闭"}`,
		);
		io.writeln("[telegram] 在聊天中发送 /clear 可开始新的 RPC 会话");
		io.writeln(
			"[telegram] 在聊天中发送 /whereami 可获取投递线程 ID",
		);
		io.writeln(
			"[telegram] 使用 /tools、/yolo 或 /cwd <path> 可更新运行时设置",
		);
		io.writeln("[telegram] 在聊天中发送 /exit 或按 Ctrl+C 可停止");
