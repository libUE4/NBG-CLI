		...
		await maybeHandleChatCommand(resolvedInput, {
			...
			setState: async (next: ChatCommandState) => {
				...
				const systemPrompt = await resolveSystemPrompt({
					...
					modelId: input.baseStartRequest.model,
			...
			schedule: {
				create: async ({ name, cronPattern, prompt }) => {
					...
					return [
						`已创建计划任务 "${created.name}"。`,
						`id=${created.scheduleId}`,
						`cron=${created.cronPattern}`,
						`nextRunAt=${created.nextRunAt || "待定"}`,
					].join("\n");
				},
				list: async () => {
					const current = await loadThreadState(
					...
					return matching
						.map((schedule) =>
							[
								`${schedule.scheduleId} ${schedule.enabled ? "[已启用]" : "[已禁用]"}`,
								`name=${schedule.name}`,
								`cron=${schedule.cronPattern}`,
								`nextRunAt=${schedule.nextRunAt || "待定"}`,
							].join("\n"),
						)
						.join("\n\n");
				},
...
>(input: {
	...
	if (!decision) {
		await postConnectorText(
			input.thread,
			input.transport ?? "",
			`"${pending.toolName}" 正在等待批准。回复 "Y" 批准，或回复 "N" 拒绝。`,
		);
		return true;
	}
	input.pendingApprovals.delete(input.thread.id);
