...
describe("cli interactive e2e", () => {
	...
	it("shows the interactive chat view on launch", () => {
		...
		expect(output).toContain("我能为你做什么？");
		expect(output).toContain("○ 规划 ● 执行");
		expect(output).toContain("已启用全部自动批准");
	});
	...
	it("toggles auto-approve-all with Ctrl+Down", () => {
		const result = runInteractiveCli([
			{ delaySeconds: INITIAL_RENDER_DELAY_SECONDS, input: "\u001b[1;5B" },
			{ delaySeconds: POST_ACTION_SETTLE_SECONDS, input: "" },
		]);
		const output = outputOf(result);
		expect(output).toContain("已启用全部自动批准");
		expect(output).toContain("全部自动批准已禁用（Ctrl+↓）");
	});
	...
	it("opens /settings and navigates tabs with Tab", () => {
		...
		expect(output).toContain("[工具] 插件 智能体 钩子 规则 MCP");
		expect(output).toContain("工具 [插件] 智能体 钩子 规则 MCP");
	});
	...
	it("closes /settings with Escape", () => {
		...
		expect(output).toContain(
			"←/→ 切换标签，Ctrl+P/N 导航，Tab/Enter 选择，Esc 关闭",
		);
		expect(output).toContain("使用 / 打开斜杠命令，@ 引用文件，Ctrl+P 打开菜单");
	});
