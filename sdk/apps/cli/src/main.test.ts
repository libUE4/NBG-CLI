...
describe("runCli lightweight command dispatch", () => {
	...
	it("ignores last-used non-third-party provider settings at startup", async () => {
		providerSettingsMocks.getLastUsedProviderSettings.mockReturnValue({
			provider: "openrouter",
			baseUrl: "https://openrouter.ai/api/v1",
			model: "openai/gpt-5",
		});
		providerSettingsMocks.getProviderSettings.mockImplementation(
			(providerId: string) =>
				providerId === "openai-compatible"
					? {
	...
	it("does not pass last-used Cline provider settings as account options", async () => {
		const clineSettings = {
			provider: "cline",
