...
describe("cli e2e", () => {
	...
	it("prints help output", () => {
		...
		expect(asText(result.stdout)).toContain("用法：");
		...
		expect(asText(result.stdout)).toContain("显示当前配置");
	}, 15_000);
	...
	it("rejects invalid timeout values", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain("无效的 timeout");
	});
	...
	it("rejects zen mode without a prompt using a prompt-specific error", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain("--zen 需要提示词。");
		expect(asText(result.stderr)).not.toContain("交互模式");
	});
	...
	it("returns an error for unknown config targets", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain(
			'config 需要以下分类之一：workflows、rules、agents、plugins、hooks、mcp、tools（收到 "unknown-target"）',
		);
	});
	...
	it("returns an error for unknown hub subcommands", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain("未知命令 'nonesuch'");
	});
	...
	it("prints empty history state from isolated storage", () => {
		...
		expect(result.status).toBe(0);
		expect(asText(result.stdout)).toContain("未找到历史记录。");
	});
	...
	it("returns an error when deleting a session without --session-id", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain(
			"history delete 需要 --session-id <id>",
		);
	});
...
Do not list this.`,
		...
		expect(result.status).toBe(0);
		expect(asText(result.stdout)).toContain("可用工作流：");
...
Do not force push.`,
		...
		expect(result.status).toBe(0);
		expect(asText(result.stdout)).toContain("已启用规则：");
...
Break work into clear steps.`,
		...
		expect(asText(textResult.stdout)).toContain("已配置 Agent：");
	...
	it("lists discovered plugins", () => {
		...
		expect(textResult.status).toBe(0);
		expect(asText(textResult.stdout)).toContain("已发现插件：");
	...
	it("lists configured mcp servers", () => {
		...
		expect(textResult.status).toBe(0);
		expect(asText(textResult.stdout)).toContain("已配置 MCP 服务器");
		expect(asText(textResult.stdout)).toContain("docs [stdio]");
		expect(asText(textResult.stdout)).toContain(
			"remote [streamableHttp]（已禁用）",
		);
	...
	it("lists available tools", () => {
		...
		const textResult = runCli(["--cwd", workspace, "config", "tools"], {
			env: {
				...createIsolatedEnv(),
				HOME: homeDir,
				CLINE_DATA_DIR: dataDir,
		...
		expect(textResult.status).toBe(0);
		expect(asText(textResult.stdout)).toContain("可用工具：");
		expect(asText(textResult.stdout)).toContain("read_files [已启用]");
		expect(asText(textResult.stdout)).toContain("spawn_agent [已启用]");
		expect(asText(textResult.stdout)).toContain("teams [已启用]");
		expect(asText(textResult.stdout)).not.toContain("submit_and_exit");
		expect(asText(textResult.stdout)).not.toContain("apply_patch");
		expect(asText(textResult.stdout)).toContain("插件工具：");
		expect(asText(textResult.stdout)).toContain("plugin_echo");
		expect(asText(textResult.stdout)).toContain("[已禁用]");

		const jsonResult = runCli(["--cwd", workspace, "config", "tools", "--json"], {
			env: {
				...createIsolatedEnv(),
				HOME: homeDir,
				CLINE_DATA_DIR: dataDir,
	...
	it("rejects invalid hook payloads", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain("无效的 hook payload");
	});
