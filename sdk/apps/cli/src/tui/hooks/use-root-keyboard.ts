...
export function useRootKeyboard(input: {
	...
	useKeyboard((key) => {
		...
		if (hasQueuedPrompts && isPreviousMainNavigationKey(key)) {
			key.preventDefault();
			if (input.autocomplete.mode) {
				input.autocomplete.close();
			}
		...
		if (isPreviousMainNavigationKey(key) && canHandleInputHistory) {
			if (
		...
		if (isNextMainNavigationKey(key) && canHandleInputHistory) {
			if (
