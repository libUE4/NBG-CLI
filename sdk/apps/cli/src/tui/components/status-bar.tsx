...
export function formatStatusBarUsageText(input: {
	...
	const tokens = `上下文 ${input.totalTokens.toLocaleString()}`;
	const usage = input.showCost
		? `${tokens} | 本会话 ${formatCost(input.totalCost)}`
		: tokens;
	const balance = input.accountBalance?.trim();
	return balance ? `${usage} | 余额 ${balance}` : usage;
}
...
export function StatusBar(props: StatusBarProps) {
	...
	const gitSuffix = hasGitDiff
		? ` | ${gitDiffStats.files} 个文件 +${gitDiffStats.additions} -${gitDiffStats.deletions}`
		: "";
	...
	return (
		<box flexDirection="column" paddingX={1}>
			...
			{autoApproveAll ? (
				<text fg={defaultFg}>
					<span fg={successColor}>{"\u23f5\u23f5"} 已启用全部自动批准</span>
					<span fg="gray"> (Ctrl+↓)</span>
				</text>
			) : (
				<text fg="gray">全部自动批准已禁用（Ctrl+↓）</text>
			)}
		</box>
	);
}
