...
function OptionRow(props: {
	...
	if (mode === "@") {
		displayName = truncateStartByWidth(opt.display, budgetAfterPrefix);
	} else {
		if (opt.description) {
			const descColumnBudget = Math.max(
				0,
				Math.floor(budgetAfterPrefix * 0.7),
			);
			const displayBudget = Math.max(
				1,
				budgetAfterPrefix - descColumnBudget - 1,
			);
			displayName = truncateEndByWidth(opt.display, displayBudget);
			if (descColumnBudget > 3) {
				descText = truncateEndByWidth(opt.description, descColumnBudget);
			}
		} else {
			displayName = truncateEndByWidth(opt.display, budgetAfterPrefix);
		}
	}
	...
	const descGap = descText
		? Math.max(
				1,
				budgetAfterPrefix - displayWidth(displayName) - displayWidth(descText),
			)
		: 0;
