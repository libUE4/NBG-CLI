	...
	if (state.step === "byo_apikey") {
		return (
			<OnboardingProviderConfigScreen
				...
				loading={state.byoSaving}
				mouse={mouse}
				values={state.byoValues}
				onFieldInput={state.handleByoFieldInput}
				onFieldFocus={state.setByoFocusedField}
	...
	if (state.step === "codex_cli_setup") {
		return (
			<OnboardingCodexCliScreen
				...
				compact={compact}
				contentWidth={contentWidth}
				authCode={state.localCliAuthCode}
				authError={state.localCliAuthError}
				authRunning={state.localCliAuthRunning}
				authStatus={state.localCliAuthStatus}
				authUrl={state.localCliAuthUrl}
				mouse={mouse}
				onAuthCodeInput={state.handleLocalCliAuthCodeInput}
				onAuthCodeSubmit={state.submitLocalCliAuthCode}
