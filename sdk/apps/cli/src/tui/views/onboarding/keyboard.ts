...
export function useOnboardingKeyboard(input: {
	...
	setByoFocusedField: (field: ProviderConfigFieldKey) => void;
	...
	cancelLocalCliAuth: () => void;
	abortOAuth: () => void;
	abortDeviceCode: () => void;
	resetAuth: () => void;
	refreshCodexCliStatus: () => void;
	startLocalCliAuth: () => void;
	submitLocalCliAuthCode: () => void;
		...
		if (input.step === "byo_apikey") {
			if (isPreviousNavigationKey(key) || isNextNavigationKey(key)) {
				...
				if (visible.length > 1) {
					...
					const nextIdx = isPreviousNavigationKey(key)
