	...
	for (const [, groupItems] of groups) {
		...
		for (const item of sortBySourceThenName(groupItems)) {
			rows.push({
				...
				item,
				indent: 4,
				rightLabel: sharedToolNames.has(item.name) ? "共享工具名" : "",
			});
		}
	}
}
...
function getPluginLoadErrorLabel(
	...
	return lines.length > 1 ? `${first}（另有 ${lines.length - 1} 行）` : first;
}
...
export function ConfigPanelContent(props: ConfigPanelProps) {
	...
	const rows = useMemo(() => {
		...
		if (activeTab === "general") {
			...
			r.push({ kind: "model" });
			r.push({ kind: "toggle", id: "mode", label: "工作模式" });
			r.push({ kind: "toggle", id: "compaction", label: "上下文压缩" });
			r.push({
				kind: "toggle",
				id: "auto-approve",
				label: "全部自动批准",
					...
					case "model":
						return (
							<box
								...
								justifyContent="space-between"
							>
								<text fg={isSel ? "cyan" : undefined}>{pfx}模型</text>
