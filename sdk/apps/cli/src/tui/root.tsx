...
function App(props: TuiProps) {
	...
	const openCheckpointRestore = useCallback(async () => {
		...
		try {
			...
			session.appendEntry({
				kind: "status",
				text: `正在恢复到检查点${restoreWorkspace ? "（对话 + 工作区）" : "（仅对话）"}...`,
			});
	...
	const promoteQueuedPrompt = useCallback(
		(promptId: string) => {
			void props
				...
				.catch((error) => {
					showToast(
						`无法引导队列消息：${error instanceof Error ? error.message : String(error)}`,
						"error",
					);
				});
		},
	...
	const agentHandlers = useAgentEventHandlers({
		...
		thinkingEnabled:
			props.config.thinking === true || Boolean(props.config.reasoningEffort),
	});
	...
	const saveQueuedPromptEdit = useCallback(
		async (promptId: string, text: string) => {
			...
			try {
				...
				showToast(
					`无法更新队列消息：${error instanceof Error ? error.message : String(error)}`,
					"error",
				);
			}
		},
