...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	if (!data) {
		return <div className="text-foreground opacity-80">子智能体状态更新不可用。</div>
	}
		...
		<div className="mb-2">
			...
			<div className="space-y-2">
				{data.items.map((entry, index) => {
					...
					const statsText = `调用 ${formatCount(entry.toolCalls)} 个工具 · ${formatCount(entry.contextTokens)} Token · ${formatCost(entry.totalCost)}`
					...
					return (
						<div
							...
							{shouldShowStats && hasDetails && (
								<button
									aria-label={isExpanded ? "隐藏子智能体输出" : "显示子智能体输出"}
