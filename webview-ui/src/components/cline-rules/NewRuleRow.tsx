...
const HOOK_TYPES = [
	{ name: "TaskStart", description: "新任务开始时执行" },
	{ name: "TaskResume", description: "任务恢复时执行" },
	{ name: "TaskCancel", description: "任务取消时执行" },
	{ name: "TaskComplete", description: "任务完成时执行" },
	{ name: "PreToolUse", description: "任意工具使用前执行" },
	{ name: "PostToolUse", description: "任意工具使用后执行" },
	{ name: "UserPromptSubmit", description: "用户提交提示词时执行" },
	{ name: "PreCompact", description: "对话压缩前执行" },
]
						...
						<form className="flex flex-1 items-center" onSubmit={handleSubmit}>
							<input
								...
								placeholder={
									isExpanded
										? ruleType === "workflow"
											? "workflow-name（.md、.txt 或无扩展名）"
											: ruleType === "skill"
												? "skill-name（字母、数字、短横线、下划线）"
												: "rule-name（.md、.txt 或无扩展名）"
										: ruleType === "workflow"
											? "新建工作流文件..."
											: ruleType === "skill"
												? "新建技能..."
