	...
	return (
		<div className="flex flex-col gap-1">
			<VSCodeRadioGroup
				...
				<VSCodeRadio value="profile">AWS 配置档案</VSCodeRadio>
			...
			apiConfiguration?.awsAuthentication === "profile" ? (
				<DebouncedTextField
					...
					placeholder="输入配置档案名称（留空使用 default）">
					<span className="font-medium">AWS 配置档案名称</span>
				</DebouncedTextField>
				...
				<DebouncedTextField
					...
					placeholder="输入 Bedrock API Key"
					type="password">
			...
			<p className="mt-1 text-sm text-description">
				{apiConfiguration?.awsUseProfile
					? "使用 ~/.aws/credentials 中的 AWS Profile 凭据。配置档案名称留空则使用 default。这些凭据仅在本地用于从此扩展发起 API 请求。"
					: "可填写上方密钥进行认证，也可以使用默认 AWS 凭据提供方，例如 ~/.aws/credentials 或环境变量。这些凭据仅在本地用于从此扩展发起 API 请求。"}
			</p>
