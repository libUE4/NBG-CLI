...
Every interaction with NBG happens within a task. Tasks are self-contained work sessions that capture your entire conversation, code changes, command executions, and decisions.

## What are Tasks?

A task begins when you submit a prompt to NBG. Your prompt defines the goal, and NBG works toward it through conversation, code changes, and tool use. The quality of your initial prompt directly affects how well NBG performs - clear, specific prompts lead to better results.
...
<Tip>
Want to get better results from NBG? Start with clear, scoped tasks and keep unrelated work in separate conversations.
</Tip>

## Scoping Your Tasks

Each task carries its own context: the conversation history, decisions made, and understanding built up over the session. How you scope your tasks directly affects how well NBG can help you.
...
Every AI model has a context window - a limit on how much information it can process at once. Think of it as NBG's working memory for the current task.

As you work, the context window fills up with:
- Your prompts and NBG's responses
- File contents NBG reads or edits
- Command outputs and tool results
- System instructions that guide NBG's behavior (including [NBG Rules](/customization/cline-rules))

When the context window approaches its limit, NBG automatically compresses older parts of the conversation to make room. This means very long tasks may lose some earlier details, though NBG preserves the most important context.
...
If your starting context seems high even for simple prompts, add a [`.clineignore`](/customization/clineignore) file to exclude dependencies, build artifacts, and other files NBG doesn't need. This compatibility file can dramatically reduce your baseline token usage.
...
Knowing when to start fresh versus continue can feel unclear at first. As you work with NBG more, you'll develop an intuition for it. Use this table as a starting point:

| Scenario | Action | Why |
|----------|--------|-----|
| Switching to a different feature | **New task** | Clean context, focused responses |
| Building on work NBG just completed | **Continue** | Shared understanding preserved |
| NBG keeps going off-track | **New task** | Fighting context wastes time |
...
| Refining NBG's last output | **Continue** | Momentum and decisions preserved |

To start a new task, click the **+** button in the NBG sidebar or use the `/newtask` command. Your file changes are preserved through [checkpoints](/core-workflows/checkpoints), and you can reference previous tasks from history anytime.

## Understanding Task Costs

Every cloud-based AI model charges for usage based on tokens, the units of text the model processes. NBG tracks these costs automatically and displays them in the task header so you can monitor spending as you work.

### How Costs Are Calculated

When you interact with NBG, the model processes:
...
Cloud providers charge per million tokens, with output tokens typically costing more than input. Some providers also support **prompt caching**, which reduces costs when the same context (like your NBG rules or large files) appears in multiple requests. NBG automatically tracks cache savings when available.
...
Not ready to pay? NBG supports several lower-cost paths:

- **Free models**: Search "free" in the model selector when using a compatible managed provider. These models display a **FREE** tag when available and work well for learning and experimentation.
...
Click the **History** button in the NBG sidebar (clock icon at the top-right) to open the history view. You'll see all your past tasks with their initial prompt, timestamp, and token usage. Each task card expands to show a preview of the conversation.

### Searching Tasks

Use the search bar at the top of the history view to find specific tasks. The fuzzy search looks across everything: your prompts, NBG's responses, code snippets, and file names.
...
NBG can resume interrupted tasks with full context:

1. Open the task from history
2. NBG loads the complete conversation
...
This works across sessions. Even if you close the editor and return days later, NBG can pick up where you left off.
