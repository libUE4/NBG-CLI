...
The NBG CLI provides a `plugin install` command to install plugins from various sources. You can also load plugins programmatically via session config.

## CLI Installation

Use the `nbg plugin install` command to install plugins from file URLs, npm, git repositories, or local paths.

```sh
nbg plugin install <source> [options]
```

The shorthand `nbg plugin i` works as well.
...
Installs the package from the npm registry into the NBG plugin directory.
...
Project-scoped plugins live in the compatible `.cline/plugins/` folder at your workspace root. The CLI auto-discovers plugins from this folder when you run NBG in that project.
...
Global plugins are discovered from the compatible `~/.cline/plugins/` path, and NBG may also discover plugins from the system Plugins folder. Use `--cwd .` when you want plugins to stay with a specific project.
