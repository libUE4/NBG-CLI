...
description: "Configure inference providers through the NBG Enterprise Console for centralized organization management"
---


Remote Provider Configuration allows administrators to centrally configure inference providers for their entire organization through the NBG Enterprise Console. This approach ensures consistent provider access, security policies, and cost management across all team members without requiring individual developer setup or infrastructure deployment.

## How Remote Configuration Works

Remote configuration operates through your NBG Enterprise Console, where administrators can:
...
NBG supports remote configuration for the following inference providers:

| Provider | Use Case | Configuration | Member Setup |
|----------|----------|---------------|--------------|
| **NBG Gateway** | Organizations using a centralized NBG-compatible provider with managed API key access | API provider selection, model access | No individual API keys needed — fully managed by organization |
...
Access the NBG Enterprise Console and configure the desired inference provider with organization-wide settings.
</Step>

<Step title="Automatic Distribution">
Provider configuration is automatically distributed to all organization members signed into NBG.
</Step>

<Step title="Member Credential Setup">
Team members add their individual credentials (API keys, AWS profiles, etc.) to connect to the configured provider. For some providers like NBG Gateway and LiteLLM (with Master Key), no individual credentials are needed.
</Step>

<Step title="Immediate Access">
Once credentials are configured, members can immediately start using the inference provider through NBG.
</Step>
</Steps>
...
4. **Start Developing**: Begin using NBG with centrally managed inference provider access
