...
Prompt Storage allows enterprises to automatically back up NBG conversation history to cloud storage (AWS S3 or Cloudflare R2). This provides a centralized repository for compliance, audit trails, and usage analysis while maintaining local storage as the primary source of truth.

## Overview

Every NBG task conversation is stored locally in the compatible storage path `~/.cline/data/tasks/<taskId>/api_conversation_history.json`. When prompt storage is enabled, a background sync worker automatically uploads these conversation files to your configured S3 or R2 bucket.
...
- ❌ Workspace files not accessed by NBG
- ❌ API keys or secrets  
  ...
  <Tab title="AWS S3">
    ...
    <Steps>
      <Step title="Create S3 Bucket">
        Create a dedicated S3 bucket for NBG conversation storage:
        
        ```bash
        aws s3 mb s3://your-nbg-prompts --region us-east-1
        ```
      ...
      <Step title="Create IAM Policy">
          ...
          "Statement": [
            {
              ...
              "Resource": "arn:aws:s3:::your-nbg-prompts/*"
            },
            {
              "Effect": "Allow",
              "Action": [
        ...
        Save this as `nbg-prompt-storage-policy.json` and create the policy:
        
        ```bash
        aws iam create-policy \
          --policy-name NBGPromptStorage \
          --policy-document file://nbg-prompt-storage-policy.json
        ```
      </Step>
      ...
      <Step title="Create IAM User">
        ...
        ```bash
        aws iam create-user --user-name nbg-prompt-uploader
        
        aws iam attach-user-policy \
          --user-name nbg-prompt-uploader \
          --policy-arn arn:aws:iam::YOUR_ACCOUNT_ID:policy/NBGPromptStorage
        
        aws iam create-access-key --user-name nbg-prompt-uploader
        ```
        
        Save the `AccessKeyId` and `SecretAccessKey` from the output.
      </Step>

      <Step title="Configure in NBG Enterprise Console">
        In your NBG Enterprise Console:
  ...
  <Tab title="Cloudflare R2">
    ...
    <Steps>
      <Step title="Create R2 Bucket">
        ...
        4. Name your bucket (e.g., `nbg-prompts`)
...
- Useful for enabling prompt storage on an existing NBG deployment
...
While prompt storage operates independently, it integrates with NBG observability:
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=BucketName,Value=your-nbg-prompts \
...
aws s3api put-bucket-logging \
  ...
  --bucket-logging-status '{
    "LoggingEnabled": {
      ...
      "TargetPrefix": "nbg-prompts-access/"
    }
  }'
```
...
<CardGroup cols={3}>
  ...
  <Card title="Telemetry" icon="chart-simple" href="/enterprise-solutions/monitoring/telemetry">
    Learn about NBG's built-in anonymous usage tracking
  </Card>
