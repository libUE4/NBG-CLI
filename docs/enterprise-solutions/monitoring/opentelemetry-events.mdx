...
description: "Complete reference of OpenTelemetry log events emitted by NBG"
---

This page documents all OpenTelemetry log events currently instrumented in NBG. These events are emitted when OpenTelemetry integration is enabled and provide detailed insights into user behavior, task execution, and system operations.
...
NBG emits events across several categories, each prefixed with a namespace:
...
| `task.rule_toggled` | NBG rule toggled on/off | rule_name, enabled, is_global |
| `task.yolo_mode_toggled` | YOLO mode toggled | enabled |
| `task.cline_web_tools_toggled` | NBG web tools setting toggled | enabled |
...
| `extension_version` | string | NBG extension version |
...
sequenceDiagram
    ...
    User->>NBG: Start Task
    NBG->>OTel: task.created
    
    User->>NBG: Submit Message
    NBG->>OTel: task.conversation_turn (user)
    
    NBG->>NBG: Process with AI
    NBG->>OTel: task.tokens
    NBG->>OTel: task.conversation_turn (assistant)
    
    NBG->>NBG: Use Tool
    NBG->>OTel: task.tool_used
    
    User->>NBG: Provide Feedback
    NBG->>OTel: task.option_selected
    
    User->>NBG: Complete Task
    NBG->>OTel: task.completed
```
