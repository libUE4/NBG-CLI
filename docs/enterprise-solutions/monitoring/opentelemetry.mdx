...
description: "Export NBG telemetry to your observability platform using OpenTelemetry Protocol (OTLP)"
---

NBG includes opt-in OpenTelemetry support for exporting metrics and logs to your own observability infrastructure using the OpenTelemetry Protocol (OTLP).
...
NBG OpenTelemetry support allows you to:
...
NBG supports OpenTelemetry's **OTLP (OpenTelemetry Protocol)** export with:

<CardGroup cols={2}>
  <Card title="Metrics Export" icon="chart-bar">
    Export metrics about NBG usage, performance, and errors
  </Card>
...
NBG supports three OTLP export protocols:
...
OpenTelemetry is configured using [Remote Configuration](/enterprise-solutions/configuration/remote-configuration/overview#how-remote-configuration-works) from your NBG Enterprise Console.

### Basic Setup

Enable OpenTelemetry, configure an OTLP endpoint, and select a protocol in the NBG Enterprise Console.
...
Configure metrics and logs independently if your collector requires separate endpoints, protocols, intervals, or batching behavior.

**Custom headers for authentication:**

Finally, if your collector needs authentication headers, you can add key value pairs in the headers section.

Add custom headers as key-value pairs when your collector requires authentication.
...
Use Datadog's OTLP endpoint and add any required API key headers in the custom headers section.

### New Relic

Export to New Relic:

Use the New Relic OTLP endpoint and configure the required authentication headers.

### Grafana Cloud

Export to Grafana Cloud:

Use the Grafana Cloud OTLP endpoint and configure the instance-specific authentication headers.
...
Once you’ve done so, if you perform some actions that trigger metrics and/or logs (such as doing a task with NBG),
...
When OpenTelemetry is enabled, NBG exports:
...
Current OpenTelemetry support in NBG:
...
<CardGroup cols={3}>
  ...
  <Card title="NBG Telemetry" icon="chart-simple" href="/enterprise-solutions/monitoring/telemetry">
