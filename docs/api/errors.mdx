...
title: "错误"
sidebarTitle: "错误"
description: "NBG API 的错误码、错误格式、mid-stream errors 和重试策略。"
---

NBG API 使用一致的 JSON 错误格式。理解这些错误有助于构建可靠集成。
...
| `402` | Payment Required | 额度不足 | 在你的 NBG API gateway 或企业控制台补充额度 |
...
def call_api_with_retry(payload, max_retries=3):
    for attempt in range(max_retries):
        response = requests.post(
            "https://<your-nbg-api>/api/v1/chat/completions",
            headers={
