...
- `cd sdk/apps/cli && bun run test:install`
- `cd sdk/apps/cli && bun run test:privacy`
- `cd sdk/apps/cli && bun run check:release`
- GitHub Actions `nbg-cli-publish` 默认 dry-run，正式发布必须手动输入 `publish`。
